    ...
    return (
        <>
            <form ref={formRef} className={`${styles.form}`} onSubmit={handleSubmitWrapper}>
                ...
                <CheckboxGroup greedOrFlex="flex" name="gender" label="Пол" direction="row" options={Object.entries(GenderOptions).map(([value, label]) => ({ label, value }))} value={formik.values.gender} onChange={(name, selectedValue) => formik.setFieldValue(name, selectedValue)} needValue error={formik.values.birth_date && formik.errors.gender} />
