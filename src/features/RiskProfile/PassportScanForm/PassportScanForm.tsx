...
export const PasportScanForm: React.FC = () => {
    ...
    const handleFileChange = (
        ...
        if (file) {
            ...
            // Если нужно для каждого поля свои превью:
    ...
    const updatePreview = (
        ...
        // Если раньше была какая-то ссылка, удаляем её
        setPreview((oldUrl) => {
            if (oldUrl) URL.revokeObjectURL(oldUrl);
            return null;
        });

        // Определяем, PDF или нет
        if (file.type === "application/pdf") {
            setIsPdf(true);
            setPreview(null); // для PDF не создаём objectURL (можно, но как миниатюру не покажешь)
        } else {
            setIsPdf(false);
            // Создаём URL для превью, если это картинка
        ...
        <form onSubmit={formik.handleSubmit} className={styles.form}>
            ...
            <ol className={styles.form__list}>
                <li className={styles.form__list__item}>
                    Разворот первой страницы паспорта полностью, не обрезанный, без засветов, все персональные данные, серия и номер паспорта должны четко читаться.
                </li>
                <li className={styles.form__list__item}>
                    Страница с регистрацией полностью, не обрезанная, без засветов, с четко читаемым адресом регистрации, серией и номером паспорта.
                    Если страница полностью заполнена печатями с регистрациями, необходимо предоставить разворот следующей страницы.
                </li>
                <li className={styles.form__list__item}>
                    Если предоставляется фото паспорта, то фото необходимо делать, положив паспорт на стол, можно подложить лист бумаги А4, пальцы рук не должны попадать в кадр.
                </li>
            </ol>
