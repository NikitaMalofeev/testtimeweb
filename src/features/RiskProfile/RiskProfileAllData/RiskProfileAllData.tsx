...
export const RiskProfileAllData = () => {
    ...
    const renderCurrencyPotentialField = (
        label: string,
        value: string | number | undefined,
        valuePersent: string
    ) => (
        <div className={styles.page__field}>
            <span className={styles.page__question}>{label}</span>
            <span className={styles.page__answer}>{formatCurrency(value)}</span>
            <span className={styles.page__answer}>
                {valuePersent || 'Нет ответа'}
            </span>
        </div>
    );
    ...
    const renderTrustedPersonField = () => (
        <div className={styles.page__field}>
            ...
            <div style={{ display: 'flex' }}>
                <span>{allUserData?.trusted_person_fio || 'Нет ответа'}</span>
            </div>
            <span>{allUserData?.trusted_person_phone || 'Нет ответа'}</span>
            <span>{allUserData?.trusted_person_other_contact || 'Нет ответа'}</span>
        </div>
    );
    ...
    return (
        <div className={styles.page}>
            ...
            <div className={styles.page__item}>
                ...
                {renderField(
                    '1. Гражданство',
                    getTranslatedValue(allSelects?.countries, allUserData?.citizenship)
                )}
                {renderTrustedPersonField()}
                {renderField(
                    '3. Статус квалифицированного инвестора',
                    allUserData?.is_qualified_investor_status ? 'Да' : 'Нет ответа'
                )}
                {renderField(
                ...
                {renderField(
                    ...
                    getTranslatedValue(
                        allSelects?.practical_investment_experience,
                        allUserData?.practical_investment_experience
                    )
                )}
                {renderField(
                    '14. Как Вы поступите, если Ваши активы из-за рыночных потрясений потеряют более 20% стоимости?',
                    getTranslatedValue(
                        allSelects?.question_assets_losing_value,
                        allUserData?.question_assets_losing_value
                    )
                )}
                {renderField(
                ...
                {renderField(
                    ...
                    getTranslatedValue(
                        allSelects?.obligations_invest_horizon,
                        allUserData?.obligations_invest_horizon
                    )
                )}
                {renderField(
                    '18. Доход от Ваших инвестиций предназначен для',
                    getTranslatedValue(
                        allSelects?.income_investments_intended,
                        allUserData?.income_investments_intended
                    )
                )}
                {renderField(
            ...
            {renderCurrencyField(
                'Деньги',
                allUserData?.risk_more_amount_expected_replenishment
            )}
            {renderField(
                'Параметры портфеля',
                getRiskProfileTranslated(allUserData?.risk_more_portfolio_parameters)
            )}
            {renderCurrencyPotentialField(
                'Возможная потеря',
                allUserData?.risk_more_possible_loss,
                allUserData?.risk_more_possible_loss_percent || 'Нет ответа'
            )}
            {renderCurrencyPotentialField(
                'Потенциальный доход',
                allUserData?.risk_more_potential_income,
                allUserData?.risk_more_potential_income_percent || 'Нет ответа'
            )}
        </div>
    );
};
